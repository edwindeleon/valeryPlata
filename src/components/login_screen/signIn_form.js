...
export default class SignInForm extends Component {
          ...
          <View style={styles.btnContainers}>
            <TouchableOpacity onPress={this._handleForgotPassword}>
              <View style={styles.forgotBtnContainer}>
                <Text style={styles.forgotBtn}>{'Olvidó su contraseña?'.toUpperCase()}</Text>
              </View>
            </TouchableOpacity>
  ...
  _handleSignIn = () => {
    this.setState({errMsg: 'Iniciando sesión...'})
    if (this.state.email.length == 0) {
      this.setState({errMsg: "Por favor escriba su email."})
    }
    else if (this.state.password.length == 0) {
      this.setState({errMsg: "Por favor escriba su contraseña."})
    }
    else {
