...
export default class AdminUsersChat extends Component {
  constructor(props) {
    super(props)
  ...
  componentDidMount() {
    ...
    firebaseApp.database().ref('chats').limitToLast(this.state.counter).on('value',
    (snapshot) => {
      ...
      firebaseApp.database().ref('chats').on('value', function(snapshot) {
              snapshot.forEach(function(childSnapshot) {
                ...
                let theKey = Object.keys(childSnapshot.val());
                console.log("El snapshot es " + childKey )
               
                firebaseApp.database().ref('users/' + childKey).on('value',
                  (snapshot) => {
                      let nombre = snapshot.val().username
  ...
  render() {
    ...
    return (
      ...
      <Text style={styles.titleTop}>{'LISTADO CHATS'}</Text>
        ...
        return (
          <View style={styles.card}>
            <View style={styles.postInfo}>
              <Text style={styles.title}>{ this.props.appStore.username }</Text>
  ...
  _onEndReached = () => {
    ...
    if (!this.state.isEmpty && !this.state.isFinished && !this.state.isLoading) {
      ...
      firebaseApp.database().ref('chats').off()
      firebaseApp.database().ref('chats').limitToLast(this.state.counter+1).on('value',
      (snapshot) => {
