...
export default class CreateNew extends Component {
  ...
  render() {
    ...
    const photo2 = this.state.imagePath2 ?
      <View style={{ flex:1, }}>
        <Image
          source={{ uri:this.state.imagePath2 }}
          resizeMode='contain'
          resizeMethod='scale'
          style={{
            height: height,
    ...
    return (
      <View style={styles.container}>
        <Spinner visible={this.state.spinnervisible} />
          <KeyboardAwareScrollView ref='scrollContent'>
            <View style={styles.divider}>
              <Text style={styles.title}>{'SUBIR NUEVA OFERTA'}</Text>
              { photo }
        ...
        <Spinner visible={this.state.spinnervisible} />
          ...
          { photo2 }
          <TouchableOpacity style={styles.btnAdd} onPress={this._takePicture2}>
            <Icon
              name={'md-camera'}
              size={30}
              color={'#fff'}
          ...
          <TouchableOpacity style={styles.btnAdd} onPress={this._selectPicture2}>
            <Icon
              name={'md-images'}
              size={30}
              color={'#fff'}
          ...
          <View style={styles.titleContainer}>
            ...
            style={styles.inputField}
            maxLength={34}
            value={this.state.postTitle2}
            onChangeText={(text) => this.setState({ postTitle2: text })}
            underlineColorAndroid='transparent'
            placeholder='Título'
            placeholderTextColor='rgba(0,0,0,.6)'
            onSubmitEditing={(event) => {
          ...
          <View style={styles.titleContainer}>
            ...
            maxLength={34}
            style={styles.inputField}
            value={this.state.postPrice2}
            onChangeText={(text) => this.setState({ postPrice2: text })}
            underlineColorAndroid='transparent'
            placeholder='Precio RD$'
            placeholderTextColor='rgba(0,0,0,.6)'
            onSubmitEditing={(event) => {
          ...
          <View style={styles.inputContainer}>
            ...
            onChangeText={(text) => this.setState({ postText2: text })}
            placeholderTextColor='rgba(0,0,0,.6)'
            />
          </View>
          <TouchableOpacity style={styles.btnAdd} onPress={this._handleNewPost2}>
            <Icon
              name={'md-add'}
              size={30}
              color={'#fff'}
  ...
  _selectPicture2 = () => {
    ...
    ImagePicker.launchImageLibrary(cam_options2, (response) => {
      if (response.didCancel) {
      }
      else if (response.error) {
      }
  ...
  _takePicture2 = () => {
    ...
    ImagePicker.launchCamera(cam_options2, (response) => {
      if (response.didCancel) {
      }
      else if (response.error) {
      }
  ...
  _handleNewPost2 = () => {
    ...
    if (this.state.imagePath2) {
      if (this.state.postTitle2.length > 0) {
        if (this.state.postPrice2.length > 0) {
          this.setState({ spinnervisible: true })
          const uid = this.props.appStore.user.uid
          const username = this.props.appStore.user.displayName
          const newPostKey = firebaseApp.database().ref('posts2').push().key
          const imageName = `${newPostKey}.jpg`
          uploadImage(this.state.imagePath2, imageName)
          .then(url => {
            fetch('https://onesignal.com/api/v1/notifications',
            {
              method: 'POST',
            ...
            const postData = {
              ...
              clientName: "",
              new_messages: 0,
              text: this.state.postText2.replace(/(\r\n|\n|\r)/gm,""),
              title: this.state.postTitle2,
              price: this.state.postPrice2,
              puid: newPostKey,
              image: url,
              imageHeight: this.state.imageHeight,
              imageWidth: this.state.imageWidth,
            ...
            .then(() => {
              this.setState({
                              postStatus: 'Producto posteado correctamente.',
                              postTitle2: '',
                              postPrice2: '',
                              postText2: '',
                              imagePath2: null,
